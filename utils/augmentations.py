...
class BackgroundNoice():

    def __init__(self, background_path, p = 0.4, sr=32000):
        birdclef2021_nocall = glob.glob(f"{background_path}/birdclef2021_nocall/*")
        birdclef2020_nocall = glob.glob(f"{background_path}/birdclef2020_nocall/*")
        freefield = glob.glob(f"{background_path}/freefield/*")
        warblrb = glob.glob(f"{background_path}/warblrb/*")
    ...
    def __call__(self, y: np.ndarray):
        return self.aug(samples=y, sample_rate=self.sr)
